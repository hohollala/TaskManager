...
export const splitTasksRawSchema = z.object({
  ...
  tasksRaw: z
    .string()
    .describe(
      "구조화된 작업 목록으로, 각 작업은 원자성을 유지하고 명확한 완료 기준을 가져야 하며, 너무 간단한 작업을 피하고, 간단한 수정은 다른 작업과 통합할 수 있으며, 작업이 너무 많지 않도록 해야 합니다. 예시: [{name: '간결하고 명확한 작업 이름으로 작업 목적을 명확히 표현해야 함', description: '상세한 작업 설명으로 구현 요점, 기술 세부사항 및 검수 기준 포함', implementationGuide: '이 특정 작업의 구체적인 구현 방법과 단계로, 이전 분석 결과를 참조하여 간결한 pseudocode 제공', notes: '추가 설명, 특별 처리 요구사항 또는 구현 제안 (선택사항)', dependencies: ['이 작업이 의존하는 선행 작업의 전체 이름'], relatedFiles: [{path: '파일 경로', type: '파일 유형 (TO_MODIFY: 수정 예정, REFERENCE: 참조 자료, CREATE: 생성 예정, DEPENDENCY: 의존 파일, OTHER: 기타)', description: '파일 설명', lineStart: 1, lineEnd: 100}], verificationCriteria: '이 특정 작업의 검증 기준과 검사 방법'}, {name: '작업2', description: '작업2 설명', implementationGuide: '작업2 구현 방법', notes: '추가 설명, 특별 처리 요구사항 또는 구현 제안 (선택사항)', dependencies: ['작업1'], relatedFiles: [{path: '파일 경로', type: '파일 유형 (TO_MODIFY: 수정 예정, REFERENCE: 참조 자료, CREATE: 생성 예정, DEPENDENCY: 의존 파일, OTHER: 기타)', description: '파일 설명', lineStart: 1, lineEnd: 100}], verificationCriteria: '이 특정 작업의 검증 기준과 검사 방법'}]"
    ),
      ...
      content: [
        {
          type: "text" as const,
          text:
            "tasksRaw 매개변수 형식 오류입니다. 형식이 올바른지 확인하고 오류를 수정해주세요. 텍스트가 너무 길어서 수정이 어려운 경우 배치로 호출하여 메시지가 너무 길어서 문제를 수정하기 어려운 상황을 방지할 수 있습니다. 오류 메시지: " +
      ...
      content: [
        {
          type: "text" as const,
          text:
            "tasks 매개변수 형식 오류입니다. 형식이 올바른지 확인해주세요. 오류 메시지: " +
